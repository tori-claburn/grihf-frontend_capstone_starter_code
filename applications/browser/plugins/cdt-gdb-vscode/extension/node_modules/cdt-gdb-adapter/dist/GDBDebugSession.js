...
class GDBDebugSession extends debugadapter_1.LoggingDebugSession {
    ...
    attachOrLaunchRequest(response, request, args) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!args.program) {
                this.sendErrorResponse(response, 1, 'The program must be specified in the request arguments');
                return;
            }
            ...
            if (request === 'attach') {
                this.isAttach = true;
                const attachArgs = args;
                yield mi.sendTargetAttachRequest(this.gdb, {
                    pid: attachArgs.processId,
                });
                this.sendEvent(new debugadapter_1.OutputEvent(`attached to process ${attachArgs.processId}`));
            }
            ...
            try {
                ...
                if (args.expression.startsWith('>') && args.context === 'repl') {
                    if (args.expression[1] === '-') {
                        yield this.gdb.sendCommand(args.expression.slice(1));
                    }
                    else {
                        yield mi.sendInterpreterExecConsole(this.gdb, {
                            threadId: frame.threadId,
                            frameId: frame.frameId,
